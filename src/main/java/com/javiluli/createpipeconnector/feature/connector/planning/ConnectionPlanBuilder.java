 ...
 *
 * <p>Centraliza la adaptacion de los extremos antes de ejecutar el buscador de
 * rutas y mantiene esa responsabilidad fuera de la fachada publica.</p>
 */
public final class ConnectionPlanBuilder {
    ...
    public static ConnectionPlan buildSegment(
        ...
        if (startIsExistingPipe && !CreatePipeBlocks.isConnectablePipe(level.getBlockState(startPos))) {
            return null;
        }
        if (endIsExistingPipe && !CreatePipeBlocks.isConnectablePipe(level.getBlockState(endPos))) {
            return null;
        }
        ...
        return createPlan(level, path, startIsExistingPipe, endIsExistingPipe);
    }
    ...
    private static ConnectionPlan createPlan(
            Level level,
            List<BlockPos> path,
            boolean startIsExistingPipe,
            boolean endIsExistingPipe
