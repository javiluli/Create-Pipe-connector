...
/**
 * Valida en servidor las interacciones, materiales y colocaciones del conector.
 */
public final class ServerPipeConnectorEvents {
    ...
    public static void onRightClickBlock(PlayerInteractEvent.RightClickBlock event) {
        ...
        if (!RouteInteractionResolver.shouldConnectorHandle(player, currentSelection, event.getLevel(), event.getPos())) {
            return;
        }

        Block heldPipeBlock = currentSelection == null
    ...
    public static boolean handlePipeTarget(Player player, ServerLevel serverLevel, PlacementTarget target) {
        ...
        plan = PipeConnectorLogic.withPipeStyleMode(plan, ConnectorSessionStore.getPipeStyleMode(player.getUUID()), currentSelection.pipeBlock());
        ...
        IncrementalPipePlacementService.enqueue(player, serverLevel, plan, currentSelection.pipeBlock(), materials);
    ...
    /** Devuelve reservas pendientes y limpia preferencias al desconectar. */
