...
public final class ServerPipeConnectorEvents {
    ...
    public static boolean handlePipeTarget(Player player, ServerLevel serverLevel, PlacementTarget target) {
        ...
        plan = PipeConnectorLogic.withPumpMode(plan, pumpMode, ConnectorSessionStore.isAutoPumpDirectionReversed(player.getUUID()));
    ...
    public static void onPlayerLoggedOut(PlayerEvent.PlayerLoggedOutEvent event) {
        Player player = event.getEntity();
        if (!player.level().isClientSide()) {
            IncrementalPipePlacementService.cancelAndRefund(player);
            PlacementAnimationPreferenceStore.clear(player.getUUID());
        }
    }
