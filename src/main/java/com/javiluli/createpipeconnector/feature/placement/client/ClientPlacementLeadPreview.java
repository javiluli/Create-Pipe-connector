...
/** Conserva el recorrido pendiente mientras el servidor lo construye. */
public final class ClientPlacementLeadPreview {
    ...
    public static void enqueue(Level level, List<PreviewPipe> pieces) {
        ...
        PENDING_PREVIEWS.addLast(new PendingPreview(List.copyOf(pieces), ++nextVersion, level.getGameTime()));
        invalidateSnapshot();
    }
    ...
    /** Invalida la instantanea compartida por las fases de render del frame. */
    private static void invalidateSnapshot() {
        cachedGameTime = Long.MIN_VALUE;
        cachedActivePreviews = List.of();
    }

    /** Datos inmutables necesarios para dibujar una ruta activa. */
    public record ActivePreview(List<PreviewPipe> pieces, int pieceIndex, int version) {
    }

    /** Estado mutable de una ruta confirmada que espera cambios del mundo. */
    private static final class PendingPreview {
        ...
        /** Crea el seguimiento local de una ruta confirmada. */
        ...
        /** Evita que un rechazo silencioso deje un fantasma permanente. */
